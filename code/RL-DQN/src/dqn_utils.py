...
class DQNet():
    ...
    def __init__(self, state_size, action_space, learning_rate, name):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.inputs = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, *self.state_size], name = 'inputs')
            self.weights = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, 1], name = 'weights')
            self.actions = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, self.action_size], name = 'actions')

            # Target network is Reward + gamma * max(Q(s', a'))
            self.target_q = tf.compat.v1.placeholder(tf.compat.v1.float32, [None], name = 'target_metwork')
            ...
            self.conv1 = tf.compat.v1.layers.conv2d(
                inputs = self.inputs, filters = 32, kernel_size = [8,8],
                strides = [4,4], kernel_initializer = tf.compat.v1.variance_scaling_initializer(),
                padding = 'valid', activation = tf.compat.v1.nn.relu,
                name = 'conv1'
            )

            self.conv2 = tf.compat.v1.layers.conv2d(
                inputs = self.conv1, filters = 64, kernel_size = [4,4],
                strides = [2,2], kernel_initializer = tf.compat.v1.variance_scaling_initializer(),
                padding = 'valid', activation = tf.compat.v1.nn.relu,
                name = 'conv2'
            )
            ...
            self.conv3 = tf.compat.v1.layers.conv2d(
                ...
                strides = [1,1], kernel_initializer = tf.compat.v1.variance_scaling_initializer(),
                padding = 'valid', activation = tf.compat.v1.nn.relu,
                name = 'conv3'
            )

            self.flatten = tf.compat.v1.layers.flatten(inputs = self.conv3)

            self.value_fully = tf.compat.v1.layers.dense(
                inputs = self.flatten, units = 1024, activation = tf.compat.v1.nn.relu,
            ...
            self.advantage_fully = tf.compat.v1.layers.dense(
                inputs = self.flatten, units = 1024, activation = tf.compat.v1.nn.relu,
                kernel_initializer = tf.compat.v1.variance_scaling_initializer(),
            ...
            '''
            self.output = self.value + tf.compat.v1.subtract(self.advantage, 
                                                tf.compat.v1.reduce_mean(self.advantage, axis = 1, keep_dims = True))
            self.output = tf.compat.v1.identity(self.output, name = 'output')
            self.q = tf.compat.v1.reduce_mean(tf.compat.v1.multiply(self.output, self.actions), axis = 1)

            self.abs_errors = tf.compat.v1.abs(self.target_q - self.q)
            self.loss = tf.compat.v1.reduce_mean(self.weights * tf.compat.v1.squared_difference(self.target_q, self.q))
            self.loss_2 = tf.compat.v1.reduce_mean(tf.compat.v1.losses.huber_loss(labels = self.target_q, predictions = self.q))
            self.optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate = self.learning_rate).minimize(self.loss)
            self.optimizer_2 = tf.compat.v1.train.AdamOptimizer(learning_rate = self.learning_rate).minimize(self.loss_2)
...
class Memory(object):
    ...
    def save_memory(self, filename, object):
        if not os.path.exists('reply_memory'):
            os.makedirs('reply_memory')
