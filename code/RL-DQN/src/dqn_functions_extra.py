...
def map_action(action, action_space):
    '''
    Map discrete action to actual car's control
    '''
...
def map_from_control(control, action_space):
    '''
    Map continuous carla controls to discrete action values
    '''
...
def process_image(queue):
    ...
    arr = np.reshape(arr, [-1, image.width])
    #arr = arr[:,:,:,3]
    #arr = arr[:,:,:,-1]
...
def compute_reward(vehicle, sensors):
    ...
    if sensors.collision_flag:
        return -1
    elif vehicle.get_transform().location.z < -10: # go to the sea
        return -1
    else:
